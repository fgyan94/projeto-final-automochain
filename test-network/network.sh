...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    infoln "Create Montadora Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-montadora.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
    ...
    infoln "Create Sistemista Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-sistemista.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
    ...
    infoln "Create Orderer Montadora Identities"
  ...
  while :
    do
      if [ ! -f "organizations/fabric-ca/montadora.ciag/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
  ...
  infoln "Generate CCP files for Montadora and Sistemista"
...
# genesis block is defined using the "CondominioOrdererGenesis" profile at the bottom
...
function createConsortium() {
  ...
  set -x
  configtxgen -profile CondominioOrdererGenesis -channelID system-channel -outputBlock ./system-genesis-block/genesis.block
  res=$?
  { set +x; } 2>/dev/null
  if [ $res -ne 0 ]; then
